...
TEST_F(ServiceLayerServerCoreTest, StreamShouldReturnFalseOnInvalidInterval) {
  bool zero_ok = service_layer_server_core_.Stream(
      "chirp_text", [](Chirp chirp) { return false; }, -1);
  EXPECT_FALSE(zero_ok);
}

// `Stream` should return false if hashtag is empty
TEST_F(ServiceLayerServerCoreTest, StreamShouldReturnFalseOnEmptyHashtag) {
  bool success = service_layer_server_core_.Stream(
      "", [](Chirp chirp) { return false; });
